...
If your trial license has expired, please visit [private trial for reading barcodes](https://www.dynamsoft.com/customer/license/trialLicense?product=dbr&package=desktop)
or [private trial for recognizing partial label text, capturing documents or other feature](https://www.dynamsoft.com/customer/license/trialLicense?product=dcv&package=desktop).
...
If you want more control when processing PDFs, such as controlling the DPI or specifying which pages to process, please refer to [this sample](https://github.com/Dynamsoft/capture-vision-nodejs-samples/tree/main/pdf-advanced).
...
you need to install `dynamsoft-capture-vision-for-node-model`. You can check the `<version>` in the SDK's [`package.json`->`peerDependencies`](https://github.com/Dynamsoft/capture-vision-nodejs-samples/blob/main/package.json#L52).
```sh
